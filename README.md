...
- **[Lazaretto](https://github.com/jamesdfinance-dev/lazaretto-mcp)** - Check whether anything you depend on is known malware before an agent installs it. `check_lockfile` matches every pinned dependency in a package-lock.json, yarn.lock, or pnpm-lock.yaml against published malicious-package advisories (OSV/OpenSSF), free and with no API key; `scan_artifact` adds deterministic behavioral analysis (credential theft, exfiltration, obfuscation, prompt injection) with file-and-line evidence.
